...
export function createBusinessDealsJobRunner(input: NodeJS.ProcessEnv = process.env): JobRunner {
  ...
  const scraperApiKey = runtime.env.SCRAPERAPI_KEY;

  if (!openAiApiKey || !scraperApiKey) {
    const missingKeys = [
      openAiApiKey ? null : "OPENAI_API_KEY",
      scraperApiKey ? null : "SCRAPERAPI_KEY"
    ].filter((key): key is string => key !== null);

    return {
      name: "business-deals",
      async run(): Promise<void> {
        console.info(`[jobs] business-deals skipped because ${missingKeys.join(" and ")} ${missingKeys.length === 1 ? "is" : "are"} not configured`);
      }
    };
  }

  const rssClient = createRssClient({ scraperApiKey });
  const discordClient = createDiscordWebhookClient({
    webhookUrl: runtime.env.DISCORD_WEBHOOK_URL
  });
