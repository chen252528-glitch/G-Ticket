...
export function createRssClient(config: RssClientConfig = {}): RssClient {
  ...
  const fetcher = config.fetcher ?? createScraperApiFetcher(config.scraperApiKey);
  ...
  return {
    async fetchFeedItems(feedUrl: string, feedName: string): Promise<RssItem[]> {
      try {
        ...
        const items = extractRssItems(document, feedName);
...
function createScraperApiFetcher(scraperApiKey: string | undefined): RssHttpFetcher {
  if (!scraperApiKey) {
    throw new Error("createRssClient requires a scraperApiKey when no custom fetcher is provided");
  }
...
function extractRssItems(document: ParsedRssDocument, feedName: string): RssItem[] {
  const channelItems = normalizeArray(document.rss?.channel?.item);

  if (channelItems.length > 0) {
    return collectValidItems(channelItems, (item) => mapRssChannelItem(item, feedName), feedName);
  }

  const atomEntries = normalizeArray(document.feed?.entry);
  return collectValidItems(atomEntries, (entry) => mapAtomEntry(entry, feedName), feedName);
}

function collectValidItems(
  candidates: unknown[],
  mapCandidate: (candidate: unknown) => RssItem,
  feedName: string
): RssItem[] {
  const items: RssItem[] = [];
  ...
  for (const [index, candidate] of candidates.entries()) {
    try {
      const parsed = rssItemSchema.safeParse(mapCandidate(candidate));
      ...
      console.warn(`[rss] skipped invalid item ${index} in feed "${feedName}"`, parsed.error.issues);
    } catch (error) {
      console.warn(
        `[rss] skipped malformed item ${index} in feed "${feedName}"`,
