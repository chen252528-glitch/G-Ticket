...
export interface DailyLowFareAverage {
  averagePriceAmountMinor: number;
  sampleDays: number;
}
...
export interface FlightPriceRepository {
  ...
  getAverageDailyLowFare(trackedDestinationId: string, sinceDays: number): Promise<DailyLowFareAverage | null>;
...
export interface BusinessDealRepository {
  deleteStaleNonQualifyingDeals(retentionDays: number): Promise<void>;
...
export function createTursoRepository(client: DatabaseClient): TursoRepository {
  return {
    ...
    async getAverageDailyLowFare(trackedDestinationId: string, sinceDays: number): Promise<DailyLowFareAverage | null> {
      const safeSinceDays = assertPositiveInteger(sinceDays, "sinceDays");

      // Average the per-day minimum instead of raw observations: each run
      // stores every returned flight (cheap and expensive), which would bias a
      // plain AVG toward whichever runs returned more results.
      const result = await client.execute({
        sql: `
          SELECT
        ...
        `,
        args: [trackedDestinationId, `-${safeSinceDays} days`]
      });
      ...
      return {
        averagePriceAmountMinor: Math.round(Number(row.average_minor)),
        sampleDays: asInteger(row.sample_days)
      };
    },
    ...
    async deleteStaleNonQualifyingDeals(retentionDays: number): Promise<void> {
      const safeRetentionDays = assertPositiveInteger(retentionDays, "retentionDays");

      // Keep recent non-qualifying rows: they are the dedupe record that stops
      // items still present in the feed from being re-extracted and re-alerted.
      await client.execute({
        sql: `
